...
const char* glswGetShader(const char* pEffectKey)
{
    ...
    if (!pLoadedEffect)
    {
        ...
        for (lineNo = 0; lineNo < lines->qty; lineNo++)
        {
            ...
            // If the line starts with "--" or "//--", then it marks a new section
            if ((blength(line) >= 2 && line->data[0] == '-' && line->data[1] == '-') ||
            		(blength(line) >= 4 && line->data[0] == '/' && line->data[1] == '/' &&
                ...
                else
                {
                    ...
                    if (gc->TokenMap)
                    {
                        ...
                        while (pTokenMapping)
                        {
                            ...
                            for (tokenIndex = 0; tokenIndex < version_tokens->qty && !directive; tokenIndex++)
                            {
                                bstring token = version_tokens->entry[tokenIndex];
                        ...
                        bstrListDestroy(version_tokens);
                    }
                }
