...
/* hume: Extension query method breaks in GL3.2+. */
static struct extension_cache_s {
    GLint extension_count;
    GLubyte **extensions;
} extension_cache;
static struct gl_version_s {
    GLuint major;
    GLuint minor;
    GLuint rev;
} gl_version;
int has_extension_or_core_cap(const char *extension, int major, int minor);
int has_extension(const char *extension);
void build_extension_cache( void );
...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if(! has_extension_or_core_cap("GL_ARB_texture_non_power_of_two", 2, 0))
		{
...
int query_tex_rectangle_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(! (has_extension_or_core_cap("GL_ARB_texture_rectangle", 2, 1) ||
              has_extension("GL_EXT_texture_rectangle") ||
              has_extension("GL_NV_texture_rectangle")     ) )
		{
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(! (has_extension_or_core_cap("GL_ARB_texture_cube_map", 1, 3) ||
              has_extension("GL_EXT_texture_cube_map"))                     )
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(! has_extension("GL_EXT_texture_compression_s3tc" ) )
		{
			/*	not there, flag the failure	*/
...
void get_gl_version(void) {
    const GLubyte *version_str;
    const GLubyte *parse_ptr;
    ...
    parse_ptr = version_str;
    GLuint *version_parts[] = { &gl_version.major, &gl_version.minor, &gl_version.rev };
    for (int i = 0; i < 3; i++) {
        GLuint *version_part = version_parts[i];
        for (; (*parse_ptr >= '0') && (*parse_ptr < '9'); ++parse_ptr) {
            *version_part *= 10;
            *version_part += (*parse_ptr - '0');
        }
        if (*parse_ptr++ != '.') break;
    }
...
int has_extension_or_core_cap(const char *extension, int majorver, int minorver) {
    if (gl_version.major == 0) {
        get_gl_version();
    }
    if (gl_version.major > majorver) return 1;
    if (gl_version.minor >= minorver) return 1;
    
    return has_extension(extension);
}

int has_extension(const char *extension) {
    build_extension_cache();
    for (GLint i = 0; i < extension_cache.extension_count; ++i) {
        if (strstr((const char *)extension_cache.extensions[i], extension)) {
            return 1;
        }
    }
    return 0;
